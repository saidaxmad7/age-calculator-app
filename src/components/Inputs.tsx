...
function Inputs({ setResult }: InputsProps) {
  ...
  const validate = () => {
    ...
    else if (year.length !== 4 || y < 1900) newErrors.year = "Must be a valid year";
    else if (y > currentYear) newErrors.year = "Must be in the past";

    if (!month || m < 1 || m > 12) newErrors.month = "Must be a valid month";
    ...
    if (!newErrors.day && !newErrors.month) {
      ...
      if (d > daysInMonth) newErrors.day = "Must be a valid day";
    }

    if (!newErrors.day && !newErrors.month && !newErrors.year) {
      const inputDate = new Date(y, m - 1, d);
      if (inputDate > today) newErrors.year = "Must be in the past";
    }
      ...
      >
        ...
        <div className={`inputs-group ${errors.month ? "error" : ""}`}>
          ...
          <input
            value={month}
            onChange={(e) =>
              e.target.value.length <= 2 && setMonth(e.target.value)
            }
            placeholder="MM"
          />
